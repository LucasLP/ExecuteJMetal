...
public abstract class AbstractMOEAD<S extends Solution<?>> implements Algorithm<List<S>> {
  ...
  public AbstractMOEAD(Problem<S> problem, int populationSize, int resultPopulationSize,
    ...
    indArray = new Solution[this.problem.getNumberOfObjectives()];
    neighborhood = new int[this.populationSize][this.neighborSize];
    idealPoint = new double[this.problem.getNumberOfObjectives()];
    nadirPoint = new double[this.problem.getNumberOfObjectives()];
    lambda = new double[this.populationSize][this.problem.getNumberOfObjectives()];
  }
  ...
  protected  void updateNeighborhood(S individual, int subProblemId, NeighborType neighborType) throws JMetalException {
    ...
    for (int i = 0; i < size; i++) {
      ...
      f1 = fitnessFunction(population.get(k), lambda[k]);
          ...
          System.out.println("K: "+k+ "\tmaxPop: "+population.size()+"\tlambdaSize: "+lambda.length);
          throw e;
      }
